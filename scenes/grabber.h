...
struct Grabber
{
   ...
   // get the grabbers current position
   m_startAzimuth = m_azimuth_axis;
   m_startShoulderRot = m_shoulderRot_angle;
   m_startRadialDist = m_radCalc_b;

   // set the grabbers destination
   m_endAzimuth = 0.0;
   m_endRadialDist = sqrt(position.x*position.x+position.z*position.z);
   m_endShoulderRot = atan2(position.x,position.z);
   ...
   {
      m_animationStep = MOVE_GRABBER;
      m_rampStep = fmin(1./ fabs(m_startRadialDist-m_endRadialDist),
            1./ fabs(m_startShoulderRot-m_endShoulderRot)) * 0.01;
   }
   // the animation starts anew
   m_ramp = 0.0;
  ...
  // m_idleSensor->schedule();
   ...
   bool animation()
    {
   if(m_ramp <= 1.0-s_epsilon) {
      // increase position in current animation phase
      m_ramp += (m_animationStep == MOVE_GRABBER) ? m_rampStep : s_rampStep;
   } else {
      // schedule next animation phase
      if(m_animationStep == MOVE_GRABBER) {
         m_ramp = 0.0;
         m_animationStep = DOWN_GRABBER;
      } else if(m_animationStep == DOWN_GRABBER) {
         m_ramp = 0.0;
         m_animationStep = UP_GRABBER;
         if(m_mode == GET_PIECE) {
            m_finger->addChild(m_gameboard->getPiece());
         } else if(m_mode == SET_PIECE) {
            int length = m_finger->getChildren();
           asg::Object piece = m_finger->getChild(length-1);
            m_finger->removeChild(piece);
           // m_gameboard->setPiece(piece);
            // the gameboard had increased the refcount of the piece
            // before handing it over to us: lower it again
            //piece->unref();
         }
      } else if(m_animationStep == UP_GRABBER) {
         m_animationStep = NO_ANIMATION;
         m_mode = INACTIVE;
         return false;
      }
   }

   // calculate the values determining the current grabber position
   switch(m_animationStep) {
      case MOVE_GRABBER:
        m_shoulderRot = grabber::makeRotation(m_shoulderRot_axis,m_startShoulderRot + (m_endShoulderRot - m_startShoulderRot)*m_ramp);
   ...
   return true;
    ...
    enum AnimationStep { NO_ANIMATION, MOVE_GRABBER, DOWN_GRABBER, UP_GRABBER };
    ...
    virtual bool handleKeyPress(visionaray::key_event const& event)
    {
        ...
        if(event.key() ==  visionaray::keyboard::ArrowRight)
        {
         ...
         if(event.key() ==  visionaray::keyboard::Enter){
            ...
            // float tr[] = {0.f,(gb.s_squareHeight+gb.s_pieceHeight)/2.f,0.f};
            // trans->translate(tr);
            // asg::Object piece = gb.m_selector_surface->getChild(0);
            // gb.m_sceneGraph->addChild(piece);
            //  gb.m_selector->removeChild(gb.m_selector_surface);
            //  trans->addChild(gb.m_selector_surface);
            // gb.m_sceneGraph->removeChild(gb.m_selector);
            // gb.m_selector->release();
            // gb.m_selector = trans;
